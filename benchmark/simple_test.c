...
/* You need to change this macro to your fs mount point*/
...
int main(int argc, char **argv) {
	...
	if ((fd = creat(TESTDIR "/file1", FILEPERM)) < 0) {
		perror("creat");
	...
	/* Open for reading */
	if ((fd = open(TESTDIR "/file1", FILEPERM)) < 0) {
		perror("open");
		exit(1);
	}
